...
def load_dataset() -> tuple[np.ndarray, np.ndarray]:
    """Carrega e monta o dataset a partir dos arquivos salvos em data/raw.

    Fluxo:
    - Percorre cada classe (pasta) em DATA_RAW
    - Para cada sequência (pasta numérica) carrega 'keypoints.npy'
    - Reamostra uniformemente os frames para ter exatamente FRAMES_PER_SEQUENCE
    - Empilha tudo em X (amostras × frames × features) e y (labels inteiras)

    Retorna:
        X: np.ndarray com shape (n_amostras, FRAMES_PER_SEQUENCE, NUM_LANDMARKS_PER_FRAME)
        y: np.ndarray com labels inteiros
    """

    X, y = [], []

    # Mapeia nomes de classes para índices inteiros
    ...
    for cls in CLASSES:
        ...
        if not os.path.exists(cls_path):
            # Se não existir pasta para uma classe, simplesmente pula
            continue

        cls_ok = 0

        # Lista das sequências (pastas) existentes para a classe
        sequences = sorted([
        ...
        for seq in sequences:
            seq_path = os.path.join(cls_path, seq, "keypoints.npy")
            ...
            # Reamostragem: garante que cada sequência tenha exatamente
            # FRAMES_PER_SEQUENCE frames, interpolando indices uniformemente
            if len(frames) == 0:
                continue

            indices = np.linspace(0, len(frames) - 1, FRAMES_PER_SEQUENCE).astype(int)
...
def save_processed(X: np.ndarray, y: np.ndarray):
    """Salva os arrays processados em DATA_PROCESSED e escreve um CSV.

    Armazena:
    - X.npy : features
    - y.npy : labels
    - metadata.csv : mapeamento sample_idx × label
    """
